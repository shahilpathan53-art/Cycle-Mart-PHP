...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($name==='' || $email==='' || $pass==='') {
    ...
    if (mysqli_fetch_assoc($exists)) {
      ...
      // Demo: store plain password (not safe, replace with password_hash later)
      $ok = mysqli_query($conn, "INSERT INTO users (role_id,name,email,password_plain) VALUES (2,'$name','$email','$pass')");
      if ($ok) {
        // Auto-login
